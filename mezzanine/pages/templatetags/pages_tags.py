...
def mezzanine_submit_row(context):
    """
...
@register.inclusion_tag("admin/includes/tasks_list.html", takes_context=True)
def tasks_list(context):
    """
    Renders the tasks for the current user as per the Workflow model.
    """
    user = context["user"]
    context["tasks"] = user.tasks_for.all()
    return context
